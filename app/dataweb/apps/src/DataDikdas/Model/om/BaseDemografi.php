...
abstract class BaseDemografi extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'DataDikdas\\Model\\DemografiPeer';
    ...
    /**
     * Initializes internal state of BaseDemografi object.
     * @see        applyDefaults()
     */
    ...
    public function setCreateDate($v)
    {
        ...
        if ($this->create_date !== null || $dt !== null) {
            ...
            if ($currentDateAsString !== $newDateAsString) {
                ...
                $this->modifiedColumns[] = DemografiPeer::CREATE_DATE;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 14; // 14 = DemografiPeer::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(DemografiPeer::DEMOGRAFI_ID)) {
            $modifiedColumns[':p' . $index++]  = '"demografi_id"';
        }
        if ($this->isColumnModified(DemografiPeer::KODE_WILAYAH)) {
            $modifiedColumns[':p' . $index++]  = '"kode_wilayah"';
        }
        if ($this->isColumnModified(DemografiPeer::TAHUN_AJARAN_ID)) {
            $modifiedColumns[':p' . $index++]  = '"tahun_ajaran_id"';
        }
        if ($this->isColumnModified(DemografiPeer::USIA_5)) {
            $modifiedColumns[':p' . $index++]  = '"usia_5"';
        }
        if ($this->isColumnModified(DemografiPeer::USIA_7)) {
            $modifiedColumns[':p' . $index++]  = '"usia_7"';
        }
        if ($this->isColumnModified(DemografiPeer::USIA_13)) {
            $modifiedColumns[':p' . $index++]  = '"usia_13"';
        }
        if ($this->isColumnModified(DemografiPeer::USIA_16)) {
            $modifiedColumns[':p' . $index++]  = '"usia_16"';
        }
        if ($this->isColumnModified(DemografiPeer::USIA_18)) {
            $modifiedColumns[':p' . $index++]  = '"usia_18"';
        }
        if ($this->isColumnModified(DemografiPeer::JUMLAH_PENDUDUK)) {
            $modifiedColumns[':p' . $index++]  = '"jumlah_penduduk"';
        }
        if ($this->isColumnModified(DemografiPeer::CREATE_DATE)) {
            $modifiedColumns[':p' . $index++]  = '"create_date"';
        }
        if ($this->isColumnModified(DemografiPeer::LAST_UPDATE)) {
            $modifiedColumns[':p' . $index++]  = '"last_update"';
        }
        if ($this->isColumnModified(DemografiPeer::SOFT_DELETE)) {
            $modifiedColumns[':p' . $index++]  = '"soft_delete"';
        }
        if ($this->isColumnModified(DemografiPeer::LAST_SYNC)) {
            $modifiedColumns[':p' . $index++]  = '"last_sync"';
        }
        if ($this->isColumnModified(DemografiPeer::UPDATER_ID)) {
            $modifiedColumns[':p' . $index++]  = '"updater_id"';
        }

        $sql = sprintf(
            'INSERT INTO "demografi" (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '"tahun_ajaran_id"':						
                        ...
                        break;
                    case '"usia_5"':						
                        $stmt->bindValue($identifier, $this->usia_5, PDO::PARAM_STR);
                        break;
                    case '"usia_7"':						
                        $stmt->bindValue($identifier, $this->usia_7, PDO::PARAM_STR);
                        break;
                    case '"usia_13"':						
                        $stmt->bindValue($identifier, $this->usia_13, PDO::PARAM_STR);
                        break;
                    case '"usia_16"':						
                        $stmt->bindValue($identifier, $this->usia_16, PDO::PARAM_STR);
                        break;
                    case '"usia_18"':						
                        $stmt->bindValue($identifier, $this->usia_18, PDO::PARAM_STR);
                        break;
                    case '"jumlah_penduduk"':						
                        $stmt->bindValue($identifier, $this->jumlah_penduduk, PDO::PARAM_STR);
                        break;
                    case '"create_date"':						
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(DemografiPeer::CREATE_DATE)) $criteria->add(DemografiPeer::CREATE_DATE, $this->create_date);
    ...
    public function getMstWilayah(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aMstWilayah === null && (($this->kode_wilayah !== "" && $this->kode_wilayah !== null)) && $doQuery) {
            ...
            /* The following can be used additionally to
                ...
                $this->aMstWilayah->addDemografis($this);
             */
        }
    ...
    public function getTahunAjaran(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aTahunAjaran === null && (($this->tahun_ajaran_id !== "" && $this->tahun_ajaran_id !== null)) && $doQuery) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aTahunAjaran->addDemografis($this);
             */
        }
    ...
    public function setVldDemografis(PropelCollection $vldDemografis, PropelPDO $con = null)
    {
        ...
        foreach ($vldDemografisToDelete as $vldDemografiRemoved) {
            $vldDemografiRemoved->setDemografi(null);
        }
    ...
    protected function doAddVldDemografi($vldDemografi)
    {
        ...
        $vldDemografi->setDemografi($this);
    }
    ...
    public function removeVldDemografi($vldDemografi)
    {
        if ($this->getVldDemografis()->contains($vldDemografi)) {
            ...
            $vldDemografi->setDemografi(null);
        }
     ...
     * @return PropelObjectCollection|VldDemografi[] List of VldDemografi objects
     */
    public function getVldDemografisJoinErrortype($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = VldDemografiQuery::create(null, $criteria);
