...
abstract class BaseJenisKeluar extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'DataDikdas\\Model\\JenisKeluarPeer';
    ...
    /**
     * Initializes internal state of BaseJenisKeluar object.
     * @see        applyDefaults()
     */
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->ptkTerdaftarsScheduledForDeletion !== null) {
                if (!$this->ptkTerdaftarsScheduledForDeletion->isEmpty()) {
                    foreach ($this->ptkTerdaftarsScheduledForDeletion as $ptkTerdaftar) {
                        // need to save related object because we set the relation to null
                        $ptkTerdaftar->save($con);
                    }
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(JenisKeluarPeer::JENIS_KELUAR_ID)) {
            $modifiedColumns[':p' . $index++]  = '"jenis_keluar_id"';
        }
        if ($this->isColumnModified(JenisKeluarPeer::KET_KELUAR)) {
            $modifiedColumns[':p' . $index++]  = '"ket_keluar"';
        }
        if ($this->isColumnModified(JenisKeluarPeer::KELUAR_PD)) {
            $modifiedColumns[':p' . $index++]  = '"keluar_pd"';
        }
        if ($this->isColumnModified(JenisKeluarPeer::KELUAR_PTK)) {
            $modifiedColumns[':p' . $index++]  = '"keluar_ptk"';
        }
        if ($this->isColumnModified(JenisKeluarPeer::CREATE_DATE)) {
            $modifiedColumns[':p' . $index++]  = '"create_date"';
        }
        if ($this->isColumnModified(JenisKeluarPeer::LAST_UPDATE)) {
            $modifiedColumns[':p' . $index++]  = '"last_update"';
        }
        if ($this->isColumnModified(JenisKeluarPeer::EXPIRED_DATE)) {
            $modifiedColumns[':p' . $index++]  = '"expired_date"';
        }
        if ($this->isColumnModified(JenisKeluarPeer::LAST_SYNC)) {
            $modifiedColumns[':p' . $index++]  = '"last_sync"';
        }

        $sql = sprintf(
            'INSERT INTO "ref"."jenis_keluar" (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '"jenis_keluar_id"':						
                        ...
                        break;
                    case '"ket_keluar"':						
                        $stmt->bindValue($identifier, $this->ket_keluar, PDO::PARAM_STR);
                        break;
                    case '"keluar_pd"':						
                        $stmt->bindValue($identifier, $this->keluar_pd, PDO::PARAM_STR);
                        break;
                    case '"keluar_ptk"':						
                        $stmt->bindValue($identifier, $this->keluar_ptk, PDO::PARAM_STR);
                        break;
                    case '"create_date"':						
    ...
    public function setPengawasTerdaftars(PropelCollection $pengawasTerdaftars, PropelPDO $con = null)
    {
        ...
        foreach ($pengawasTerdaftarsToDelete as $pengawasTerdaftarRemoved) {
            $pengawasTerdaftarRemoved->setJenisKeluar(null);
        }
    ...
    protected function doAddPengawasTerdaftar($pengawasTerdaftar)
    {
        ...
        $pengawasTerdaftar->setJenisKeluar($this);
    }
    ...
    public function removePengawasTerdaftar($pengawasTerdaftar)
    {
        if ($this->getPengawasTerdaftars()->contains($pengawasTerdaftar)) {
            ...
            $pengawasTerdaftar->setJenisKeluar(null);
        }
    ...
    public function setPtkTerdaftars(PropelCollection $ptkTerdaftars, PropelPDO $con = null)
    {
        ...
        foreach ($ptkTerdaftarsToDelete as $ptkTerdaftarRemoved) {
            $ptkTerdaftarRemoved->setJenisKeluar(null);
        }
    ...
    protected function doAddPtkTerdaftar($ptkTerdaftar)
    {
        ...
        $ptkTerdaftar->setJenisKeluar($this);
    }
    ...
    public function removePtkTerdaftar($ptkTerdaftar)
    {
        if ($this->getPtkTerdaftars()->contains($ptkTerdaftar)) {
            ...
            $this->ptkTerdaftarsScheduledForDeletion[]= $ptkTerdaftar;
            $ptkTerdaftar->setJenisKeluar(null);
        }
    ...
    public function setRegistrasiPesertaDidiks(PropelCollection $registrasiPesertaDidiks, PropelPDO $con = null)
    {
        ...
        foreach ($registrasiPesertaDidiksToDelete as $registrasiPesertaDidikRemoved) {
            $registrasiPesertaDidikRemoved->setJenisKeluar(null);
        }
    ...
    protected function doAddRegistrasiPesertaDidik($registrasiPesertaDidik)
    {
        ...
        $registrasiPesertaDidik->setJenisKeluar($this);
    }
    ...
    public function removeRegistrasiPesertaDidik($registrasiPesertaDidik)
    {
        if ($this->getRegistrasiPesertaDidiks()->contains($registrasiPesertaDidik)) {
            ...
            $registrasiPesertaDidik->setJenisKeluar(null);
        }
